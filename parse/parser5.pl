...
my $con = Grids::Console->new(
    ...
    handlers => {
		...
		select_group => \&google_select_group,
		fetch_thread => \&google_fetch_thread,
		search => \&google_search_groups,
    },
);
...
my $agent = new WWW::Google::Groups(
...
sub google_select_group {
	my $group_name = shift;
	
	$group = $agent->select_group($group_name);
	
	$con->print("Accessing group $group_name");
}

sub google_fetch_thread {
	my $thread_num = shift;
	
	$group->starting_thread($thread_num);
	
	$con->print("Set thread to $thread_num");
}

sub google_search_groups {
	my @search_terms = @_;
	my $search_term = "";

	foreach my $temp_str (@search_terms) {
		$search_term = $search_term . $temp_str;
	}
	
	my $result = $agent->search( 
								 query => $search_term,
								 limit => 1,
								 );
	my $thread;
	...
	while( $thread = $result->next_thread ){
		while( $article = $thread->next_article()) {
			$con->print( $thread->title);
			$con->print( $article->body());
		}
	}
}
